...
public class NacosRegistryFactory implements RegistryFactory {
    ...
    private static final Logger logger = LoggerFactory.getLogger(NacosRegistryFactory.class);
    ...
    public RegistryService getRegistry(RegistryConfig config) {
        ...
        if (registryService != null && ((NacosRegistryService) registryService).isConnected()) {
        ...
        try {
            ...
            if (registryService == null) {
                registryService = new NacosRegistryService(config);
                if(registryService == null){
                    throw new IllegalArgumentException(key + " is not a valid nacos address, create failed！");
                }
                CLIENT_CACHE.put(key, registryService);
            }
        }catch (Exception e){
            logger.error("Failed to create nacos client for {}", key, e);
        ...
        try {
            ...
            CLIENT_CACHE.forEach((key, registry) -> {
                try {
                    ...
                    registry.destroy();
                    // 记录销毁成功的日志信息
                    logger.info("Closed nacos client: {}", key);
                } catch (Exception e) {
                    // 如果销毁失败，记录错误日志信息
                    logger.error("Failed to close nacos client: {}", key, e);
                }
            });
            // 清空客户端缓存
            CLIENT_CACHE.clear();
